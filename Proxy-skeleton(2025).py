...
try:
  ...
  # ~~~~ INSERT CODE ~~~~
  serverSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # Create TCP socket for the proxy server
  # ~~~~ END CODE INSERT ~~~~
...
while True:
  ...
  try:
    # ~~~~ INSERT CODE ~~~~
    clientSocket, clientAddress = serverSocket.accept() # store incoming client connection after accepting
    # ~~~~ END CODE INSERT ~~~~
  ...
  try:
    ...
    try:
      ...
      # ~~~~ INSERT CODE ~~~~
      response_data = b"" # buffer for response
      while True:
      ...
      # ~~~~ INSERT CODE ~~~~
      cacheFile.write(response_data) # Save response in cache for use next time
      # ~~~~ END CODE INSERT ~~~~
      cacheFile.close()
